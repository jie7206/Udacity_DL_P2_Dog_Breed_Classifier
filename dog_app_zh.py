...
print("人图像中检测到人脸的概率：{:.2f}%".format(detect_human_num/len(human_files_short)*100))
...
print("狗图像中检测到人脸的概率：{:.2f}%".format(detect_dog_num/len(dog_files_short)*100))
...
# __回答:__
# 
# 如果用户能提供含有清晰面部特征的人脸图像当然是最好，可以提高算法的准确率，但是，如果实在是没有的话，通过feature extraction技术，设定CNN的kernel为3x3 sharpen kernel，例如：[[0,-1,0],[-1,5,-1],[0,-1,0]]，利用此方法，即使面部特征并不清晰，但检测到人脸应该是可以的，如果还不行的话，或许可以使用keras.preprocessing.image.ImageDataGenerator所提供的方法，通过随机的缩放、旋转、平移和翻转影像等方式，以便增加更多样本数，这样也能对辨识率的提升有所帮助。
...
print("人图像中检测到狗脸的概率：{:.2f}%".format(detect_human_num/len(human_files_short)*100))
...
print("狗图像中检测到狗脸的概率：{:.2f}%".format(detect_dog_num/len(dog_files_short)*100))
...
# __回答:__ 
# 
# 我是用自己的方法搭建以下的卷积神经网络，主要是参考Alexis老师在以Keras实现卷积神经网络中的第18堂课(Keras中的CNN实例)里建议的做法。
# 
# 每一层的用意说明如下：
# 1. 1~6层主要是使用Conv2D+MaxPooling2D函数重复3次，使得图像的长度和宽度减小，并使得厚度从16增加到64，以便实现卷积神经网络的特性。
# 2. 第7层使用Dropout函数以避免过拟合，毕竟总参数高达12,890,037，这点使我必须加以注意。
# 3. 第8层使用Flatten函数将所有的Weights和Biases扁平化，将其维度降成1维，以作为从卷积层到全连接层的过渡，这样我们才能对图像进行分类。
# 4. 第9层使用Dense函数建立一个全连接层以用于分类，节点数设为256，不是很高，主要是考虑到总参数可能过多而影响计算效能及容易产生过拟合问题。
# 5. 第10层使用Dense函数搭配softmax激活函数以建立输出层，因为总共有133种狗狗品种，因此节点数设为133，另外，使用softmax函数使其输出成0到1且总合为1的值，以便能以概率方式输出分类结果。
...
test_predicted_file = test_files[101]
print("Real file path: ", test_predicted_file)
print("Predicted Result: ", VGG16_predict_breed(test_predicted_file))
...
print(valid_InceptionV3.shape)
print(test_InceptionV3.shape)
...
# __回答:__ 
# 
# 该网络架构使用预训练的 InceptionV3 模型作为图像特征提取器，其中 InceptionV3 最后一层卷积层的输出被直接输入到我们的模型里。因此，我们只需要添加一个全局平均池化层以及一个全连接层，根据所要分类的种类数设置全连接层的节点数，最后使用 softmax 激活函数，返回预测的结果。
# 
# 使用这个预训练模型会比第三步(从头开始创建一个CNN)中的模型还要成功，以我选择的预训练模型InceptionV3来说明，主要是因为InceptionV3的卷积层、池化层、Dropout以及全连接层加起来总共有40多层，因此提取图像特征的能力远远超过我自己设计的模型(10层)，而且InceptionV3是使用ImageNet里超过1400万张的图像来训练模型，不过这样的模型通常需要使用带GPU加速的计算器运算好几个星期的时间，因此，使用迁移学习相对来讲会高效许多，尤其是当我们没有足够多的数据可供训练，或者没有足够的运算资源时，迁移学习是个很不错的选择，并且更令人振奋鼓舞的是，即使我们自己的图像不在ImageNet中1000种的分类标签里面，迁移学习仍然比我们从头开始训练一个CNN效果要来得好很多，而且使用的方法也不困难，只要将原本模型里最后一个用来分类的全连接层去掉，用我们自己根据分类的类别数来定义的全连接层来替换即可。另外，补充一点，使用预训练好的数据集主要也是为了节省我们宝贵的时间，如果以这个项目的例子来说，总共有6680张狗狗的图片需要训练，如果每一张都要我们自己重头使用InceptionV3模型来计算，将会耗费相当长的时间，一张图片如果需要计算一分钟，那么一个epoch需要耗费4天半的时间，因此，有必要使用预训练的数据，将从图像中提取的特征向量(Bottleneck Features)直接当作我模型的输入层传入，以加速模型的训练，并且能更加提高迁移学习的效率。
...
def InceptionV3_predict_breed(img_path):
    ...
    # 返回此模型预测的狗的品种
    return dog_names[np.argmax(predicted_vector)][3:]


# In[14]:


test_predicted_file = test_files[120]
print("Real file path: ", test_predicted_file)
print("Predicted Result: ", InceptionV3_predict_breed(test_predicted_file))
...
# 狗品种名称输出
def show_breed(str):
    return str[4:].replace("_"," ").title()

def show_images_table(image_path, predict_code, title='Default title', 
                      dog_imgs_num=2, img_size=(200,200)):
    """将预测结果以图像显示
    # 参数说明
        image_path: 输入的图档路径
        predict_code: CNN网路计算结果
        dog_imgs_num: 取几张狗图像参考图
        img_size: 输出图像的尺寸
    """   
    if predict_code:
        fig, axs = plt.subplots(figsize=(20,10), nrows=dog_imgs_num+1, ncols=1)
        axs[0].set_title(title)
        axs[0].imshow(image.load_img(image_path, target_size=img_size))
        axs[0].axis('off')
        dog_imgs = glob("/data/dog_images/train/{}/*".format(predict_code))
        for n, idx in enumerate(np.random.choice(len(dog_imgs),
                                                 dog_imgs_num,replace=False)):
            axs[n+1].set_title("Similar Image")
            axs[n+1].imshow(image.load_img(dog_imgs[idx], target_size=img_size))
            axs[n+1].axis('off')
    else:
        fig, ax = plt.subplots(figsize=(4,4), nrows=1, ncols=1) 
        ax.set_title(title)
        ax.imshow(image.load_img(image_path, target_size=img_size))
        ax.axis('off')
        
    plt.tight_layout()
    
    
# 输入为图像的路径，它能够区分图像是否包含一个人、狗或两者都不包含
def image_detector(image_path):
    if face_detector(image_path):
        predict_code = InceptionV3_predict_breed(image_path)
        title = 
        "Hello, Human! You look like a ...{}".format(show_breed(predict_code))
        show_images_table(image_path, predict_code, title)
    elif dog_detector(image_path):
        predict_code = InceptionV3_predict_breed(image_path)
        title = "The dog breed is : {}".format(show_breed(predict_code))
        show_images_table(image_path, predict_code, title)
    else: 
        title = "Error! No human or dog face found!"
        show_images_table(image_path, None, title)
...
# 1. 输出结果比你预想的要好吗？或者更糟？
# 
#    回答：在辨识人脸方面几乎都能分辨出是否为人脸，不过，该人脸和哪一种狗很像就不是那么准确，亦即看不出有何具体相像的特征。另外，在辨识狗脸方面表现较为杰出，基本上能达到80%以上的准确率(指正确分辨出狗的品种而言)，同时，几乎不会将任何一支猫误判为狗，这一点令人满意。
# 
#    
# 2. 提出至少三点改进你的模型的想法。
# 
#    回答：1. 增加更多的训练样本，例如5万张以上。 2. 狗的品种样本能更多一些，这样如果现实中的狗不在这133种当中，也能被辨认出。 3. 多增加全连接层的层数与节点数，以帮助模型能更准确分类图像。

# In[20]:


## TODO: 在你的电脑上，在步骤6中，至少在6张图片上运行你的算法。
photos = ['face_1','dog_1','cat_1','face_2','dog_2','cat_2','face_3','dog_3','cat_3']
image_detector("./myimages/{}.jpg".format(photos[0]))


# In[21]:


image_detector("./myimages/{}.jpg".format(photos[1]))


# In[22]:


image_detector("./myimages/{}.jpg".format(photos[2]))


# In[23]:


image_detector("./myimages/{}.jpg".format(photos[3]))


# In[24]:


image_detector("./myimages/{}.jpg".format(photos[4]))


# In[25]:


image_detector("./myimages/{}.jpg".format(photos[5]))


# In[26]:


image_detector("./myimages/{}.jpg".format(photos[6]))


# In[27]:


image_detector("./myimages/{}.jpg".format(photos[7]))


# In[28]:


image_detector("./myimages/{}.jpg".format(photos[8]))
